<div>
    ...
    @if(strlen($query) >= 2 && $suggestions->isNotEmpty())
        <div class="mb-4 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
            <div class="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                @foreach($suggestions as $s)
                    <button
                        wire:click="addCode('{{ $s->Code }}')"
                        class="flex items-center justify-between rounded border border-gray-200 p-2 text-left hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
                    >
                        <span>
    ...
    <div class="mb-6 flex flex-wrap gap-2">
        @forelse($selectedCodes as $code)
            <div class="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1 text-sm dark:border-gray-700 dark:bg-gray-900">
